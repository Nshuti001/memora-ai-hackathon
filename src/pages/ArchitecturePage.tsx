...
const nodes: ArchNode[] = [
  { id: 'react', label: 'React Frontend', icon: Code, desc: 'TypeScript + TailwindCSS SPA with real-time WebSocket updates and optimistic UI.', layer: 'Client', color: '#22d3ee' },
  { id: 'gateway', label: 'API Gateway', icon: Network, desc: 'AWS API Gateway routes REST and WebSocket traffic with rate limiting and JWT auth via Amazon Cognito.', layer: 'Edge', color: '#34d399' },
    ...
    <div className="pt-32 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="max-w-3xl mx-auto text-center mb-16">
          <span className="text-sm font-semibold text-brand-400 tracking-wide uppercase">Architecture</span>
        ...
        <div ref={reveal.ref} className={`glass-strong rounded-3xl p-6 sm:p-10 mb-8 ${reveal.revealed ? 'revealed' : ''} reveal`}>
              ...
              return (
                <div key={layer}>
                  <div className="flex items-center gap-2 mb-3">
                    <span className="text-xs font-mono text-ink-500 uppercase tracking-wider">{layer} Layer</span>
                    ...
                    {layerNodes.map((n) => (
                      <button
                        ...
                        className={`group relative glass rounded-2xl p-4 text-left transition-all duration-300 hover:-translate-y-1 ${
                          selected === n.id ? 'ring-1 ring-brand-400/40 bg-white/[0.05]' : 'hover:bg-white/[0.04]'
                        }`}
                      >
                        <div className="flex items-center gap-3 mb-2">
                          <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: `${n.color}15`, border: `1px solid ${n.color}30` }}>
                            <n.icon className="w-5 h-5" style={{ color: n.color }} strokeWidth={1.75} />
                          </div>
                          <span className="text-sm font-semibold text-white">{n.label}</span>
                        </div>
                        <p className="text-xs text-ink-400 leading-relaxed line-clamp-2">{n.desc}</p>
                        {selected === n.id && (
                          <div className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-brand-400 ring-2 ring-ink-950" />
                        )}
                      </button>
                    ))}
                  </div>
          ...
          <div className="mt-8 pt-6 border-t border-white/[0.06]">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs font-mono text-ink-500 uppercase tracking-wider">Data Flow</span>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              {['React', 'API Gateway', 'Lambda', 'Bedrock', 'CockroachDB', 'Vector Index', 'MCP Server', 'Agent Skills'].map((step, i, arr) => (
                <div key={step} className="flex items-center gap-2">
                  <span className="text-xs font-medium text-ink-300 glass rounded-lg px-2.5 py-1.5">{step}</span>
                  {i < arr.length - 1 && <ArrowRight className="w-3 h-3 text-ink-600" />}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="glass rounded-2xl p-6 mb-8">
          <div className="flex items-start gap-4">
            <div className="shrink-0 w-14 h-14 rounded-2xl flex items-center justify-center" style={{ backgroundColor: `${selectedNode.color}15`, border: `1px solid ${selectedNode.color}30` }}>
              <selectedNode.icon className="w-7 h-7" style={{ color: selectedNode.color }} strokeWidth={1.75} />
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                <h3 className="font-display text-xl font-semibold text-white">{selectedNode.label}</h3>
                <span className="text-xs font-mono text-ink-500 px-2 py-0.5 rounded bg-white/[0.04]">{selectedNode.layer} Layer</span>
              </div>
              <p className="text-ink-300 leading-relaxed">{selectedNode.desc}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-12">
          {[
            ...
            <div key={i} className="glass rounded-xl p-3 flex items-center gap-2.5 hover:bg-white/[0.04] transition-colors">
              <t.icon className="w-4 h-4 text-brand-400 shrink-0" />
              <span className="text-xs font-medium text-ink-200 truncate">{t.name}</span>
            </div>
          ))}
        </div>
